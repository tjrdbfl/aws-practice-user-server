...
export default function userDetailPage(props: any) {
  ...
  const nameCheck = useRef<HTMLInputElement>(null);
  const phoneCheck = useRef<HTMLInputElement>(null);
  const jobCheck = useRef<HTMLInputElement>(null);
  ...
  if (isDirty) {
    console.log(pwCheck.current?.value)
    setValue("password", pwCheck.current?.value || "");
    setValue("name", nameCheck.current?.value || "");
    setValue("phone", phoneCheck.current?.value || "");
    setValue("job", jobCheck.current?.value || "");
  }
  else{
    ...
    <>
      <form onSubmit={handleSubmit(onSubmit)} className=" max-w-xl mx-auto mb-10 mt-10">
        ...
        <div className="editor mx-auto w-11/12 flex flex-col text-gray-800 border border-gray-300 p-7 shadow-lg">
          ...
          <input
            {...register('name', { required: true , minLength: { value: 3, message: "3글자 이상 입력해주세요" } })}
            ref={nameCheck}
            className="input_className"
            placeholder="Name"
            type="text"
