...
const WriterArticlePage: NextPage = () => {
  ...
  onChange={(event) => {setArticle({...article,boardId:parseInt(event.target.value)})}}
>
  ...
  <option value="2">Review</option>
      ...
      <div className="editor mx-auto w-10/12 flex flex-col text-gray-800 border border-gray-300 p-4 shadow-lg max-w-2xl">
        ...
        <input className="title bg-gray-100 border border-gray-300 p-2 mb-4 outline-none" placeholder="Title" type="text" name="title"
          onChange={(event) => {setArticle({...article,title:event.target.value})}} />
        <textarea className="description bg-gray-100 sec p-3 h-60 border border-gray-300 outline-none" placeholder="Describe everything about this post here" name="content"
          onChange={(event) => {setArticle({...article,content:event.target.value})}}></textarea>
        {/* <!-- icons --> */}
